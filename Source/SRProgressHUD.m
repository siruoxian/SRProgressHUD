...
#define SRMainThreadAssert() NSAssert([NSThread isMainThread], @"SRProgressHUD needs to be accessed on the main thread.");

CGFloat const SRProgressMaxOffset = 1000000.f;

static const CGFloat SRDefaultPadding = 4.f;
static const CGFloat SRDefaultLabelFontSize = 16.f;
static const CGFloat SRDefaultDetailsLabelFontSize = 12.f;


@interface SRProgressHUD () {
    // Deprecated
...
@interface SRProgressHUDRoundedButton : UIButton
@end


@implementation SRProgressHUD

#pragma mark - Class methods

+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    SRProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    SRProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (SRProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            SRProgressHUD *hud = (SRProgressHUD *)subview;
...
- (void)commonInit {
    ...
    _animationType = SRProgressHUDAnimationFade;
    _mode = SRProgressHUDModeIndeterminate;
    _margin = 20.0f;
    _opacity = 1.f;
...
- (void)animateIn:(BOOL)animatingIn withType:(SRProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == SRProgressHUDAnimationZoom) {
        type = animatingIn ? SRProgressHUDAnimationZoomIn : SRProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == SRProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == SRProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == SRProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == SRProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
...
- (void)done {
    ...
    SRProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<SRProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    SRBackgroundView *backgroundView = [[SRBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = SRProgressHUDBackgroundStyleSolidColor;
    ...
    SRBackgroundView *bezelView = [SRBackgroundView new];
    ...
    label.font = [UIFont boldSystemFontOfSize:SRDefaultLabelFontSize];
    label.opaque = NO;
    ...
    detailsLabel.font = [UIFont boldSystemFontOfSize:SRDefaultDetailsLabelFontSize];
    detailsLabel.opaque = NO;
    detailsLabel.backgroundColor = [UIColor clearColor];
    _detailsLabel = detailsLabel;
    
    UIButton *button = [SRProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
    button.titleLabel.textAlignment = NSTextAlignmentCenter;
    button.titleLabel.font = [UIFont boldSystemFontOfSize:SRDefaultDetailsLabelFontSize];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[SRRoundProgressView class]];
    
    SRProgressHUDMode mode = self.mode;
    if (mode == SRProgressHUDModeIndeterminate) {
    ...
    else if (mode == SRProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[SRBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == SRProgressHUDModeDeterminate || mode == SRProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[SRRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == SRProgressHUDModeAnnularDeterminate) {
            [(SRRoundProgressView *)indicator setAnnular:YES];
        }
    }
    else if (mode == SRProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == SRProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
#pragma clang diagnostic pop
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[SRProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[SRProgressHUD class]]];
#endif
        
        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[SRRoundProgressView class]]) {
        SRRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [SRRoundProgressView appearanceWhenContainedIn:[SRProgressHUD class], nil];
#else
        appearance = [SRRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[SRProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((SRRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((SRRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[SRBarProgressView class]]) {
        SRBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [SRBarProgressView appearanceWhenContainedIn:[SRProgressHUD class], nil];
#else
        appearance = [SRBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[SRProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((SRBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((SRBarProgressView *)indicator).lineColor = color;
        }
    } else {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
    SRBackgroundView *bezelView = self.bezelView;
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ? SRDefaultPadding : 0.f;
...
- (void)setMode:(SRProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == SRProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
@implementation SRRoundProgressView
...
@implementation SRBarProgressView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_7_0) {
            _style = SRProgressHUDBackgroundStyleBlur;
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
            ...
            _style = SRProgressHUDBackgroundStyleSolidColor;
...
- (void)setStyle:(SRProgressHUDBackgroundStyle)style {
    if (style == SRProgressHUDBackgroundStyleBlur && kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNumber_iOS_7_0) {
        style = SRProgressHUDBackgroundStyleSolidColor;
    }
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
...
- (void)updateForBackgroundStyle {
    SRProgressHUDBackgroundStyle style = self.style;
    if (style == SRProgressHUDBackgroundStyleBlur) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == SRProgressHUDBackgroundStyleBlur) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
            self.backgroundColor = self.color;
        } else {
#if !TARGET_OS_TV
...
@implementation SRProgressHUD (Deprecated)

#pragma mark - Class

+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
    NSArray *huds = [SRProgressHUD allHUDsForView:view];
    for (SRProgressHUD *hud in huds) {
        hud.removeFromSuperViewOnHide = YES;
        [hud hideAnimated:animated];
    }
    return [huds count];
...
- (void)showAnimated:(BOOL)animated whileExecutingBlock:(dispatch_block_t)block onQueue:(dispatch_queue_t)queue completionBlock:(nullable SRProgressHUDCompletionBlock)completion {
...
- (BOOL)dimBackground {
    SRBackgroundView *backgroundView = self.backgroundView;
    UIColor *dimmedColor =  [UIColor colorWithWhite:0.f alpha:.2f];
    return backgroundView.style == SRProgressHUDBackgroundStyleSolidColor && [backgroundView.color isEqual:dimmedColor];
}

- (void)setDimBackground:(BOOL)dimBackground {
    SRMainThreadAssert();
    self.backgroundView.style = SRProgressHUDBackgroundStyleSolidColor;
...
@implementation SRProgressHUDRoundedButton
